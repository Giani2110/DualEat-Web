...
  const sanitizedLocalName = local.name.trim().toLowerCase().replace(/ /g, '_').replace(/[^a-z0-9_]/g, '');

  // 3. Construir la nueva URL con el nombre del local sanitizado.
  const urlDelMenu = `http://localhost:5000/menu/${sanitizedLocalName}`;

  // 4. Generar el código QR como un Data URL.
  const qrCodeDataUrl = await qrcode.toDataURL(urlDelMenu);

  // 5. Devolver la respuesta formateada.
  return {
    qrCodeDataUrl,
