...
  const images: string[] = useMemo(() => [Restaurant, Restaurant2, Restaurant3], []);
...
  const [prevImageIndex, setPrevImageIndex] = useState<number>(0); // Nuevo estado para la imagen anterior
  const [imagesPreloaded, setImagesPreloaded] = useState<boolean>(false);
  const [transitioning, setTransitioning] = useState<boolean>(false); // Para controlar la transición de opacidad
...
        setPrevImageIndex(currentImageIndex); // Guarda la imagen actual como "anterior"
...
              style={{
                backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.6) 100%), url(${images[prevImageIndex]})`,
                borderTopLeftRadius: '80px',
                borderBottomLeftRadius: '80px',
                transition: 'opacity 1s ease-in-out',
                opacity: transitioning ? 0 : 1, // Desvanece la imagen anterior
              }}
            ></div>
            {/* Nueva imagen - aparece sobre la anterior */}
            <div
              className="absolute inset-0 bg-cover bg-center"
              style={{
...
                borderBottomLeftRadius: '80px',
                transition: 'opacity 1s ease-in-out',
                opacity: transitioning ? 1 : 1, // La nueva imagen siempre está opaca 1 una vez cargada y activa
              }}
              onTransitionEnd={() => setTransitioning(false)} // Cuando la transición termina, resetea el estado
            ></div>
          </>
        ) : (
