      ...
      <section className="mt-10 ps-[50px] w-[80%] flex flex-col mx-auto">
        ...
        {!categorySlug && recommendedCommunities.length > 0 && (
          <div className="mt-10 w-[80%]">
            <h3 className="text-[18px] text5 Arvo-Bold mb-4 tracking-tight">
        ...
        {!categorySlug && trendingCommunities.length > 0 && (
          <div className="mt-10 w-[80%]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
              {trendingCommunities.slice(0, visibleTrendingCount).map((community) => (
                <CommunityCard key={community.id} community={community} />
              ))}
            </div>
            ...
            {communityTags.map((ctag) => {
              const communitiesInTag = communities.filter(
                (c) =>
                  ...
                  !renderedCommunityIds.has(c.id)
              );

              communitiesInTag.forEach((c) => renderedCommunityIds.add(c.id));
