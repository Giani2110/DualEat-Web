...
const SortableIngredient = ({
  ...
  const selectIngredient = (ingredient: Ingredient) => {
    updateIngredient(formIngredient.id, "name", ingredient.name);
    setShowSuggestions(false);
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-3 w-full ${isDragging ? "z-10" : ""}`}
    >
      {/* Drag handle */}
      <div
        {...attributes}
...
const IngredientsCard = () => {
  ...
  const [formIngredients, setFormIngredients] = useState<FormIngredient[]>([
    { id: crypto.randomUUID(), quantity: "", unitId: "", name: "" },
  ]);
